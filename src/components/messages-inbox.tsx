...
export function MessagesInbox() {
  ...
  return <main className="acheix-neon-screen min-h-screen pb-24 text-white"><div className="mx-auto max-w-2xl px-4 pt-8">
    ...
    {selecting?<div className="acheix-glass-panel mt-5 flex flex-wrap items-center gap-2 rounded-3xl p-3">
...
function Row({item,selecting,checked,toggle}:{item:Conversation;selecting:boolean;checked:boolean;toggle:(id:string)=>void}){
  ...
  return selecting?<button type="button" onClick={()=>toggle(item.id)} className={`acheix-glass-panel mb-3 flex w-full gap-3 rounded-3xl p-3 text-left ${checked?"ring-1 ring-yellow-300/60":""}`}>{content}</button>:<Link href={item.target.href as any} prefetch={false} className={`acheix-glass-panel mb-3 flex gap-3 rounded-3xl p-3 transition hover:-translate-y-0.5 ${item.direction==="RECEIVED"?"ring-1 ring-emerald-400/25":""}`}>{content}</Link>;
}
function FilterButton({active,click,label,count}:{active:boolean;click:()=>void;label:string;count:number}){return <button type="button" onClick={click} className={`h-12 shrink-0 rounded-full border px-5 text-lg font-black shadow-lg transition ${active?"border-yellow-200 bg-yellow-300 text-black shadow-yellow-300/20":"border-yellow-300/20 bg-white/[0.05] text-white backdrop-blur-xl"}`}>{label} <span className="ml-1 text-sm">{count}</span></button>}
function Thumb({item}:{item:Conversation}){return item.target.imageUrl?<img src={item.target.imageUrl} alt="" className="h-20 w-20 shrink-0 rounded-2xl bg-black object-contain"/>:<span className="grid h-20 w-20 shrink-0 place-items-center rounded-2xl bg-neutral-900 text-2xl font-black">{item.target.kind==="SERVICE"?"S":item.target.title.charAt(0).toUpperCase()}</span>}
